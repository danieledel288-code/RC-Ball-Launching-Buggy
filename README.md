...
- **`train/train.ino`** - Demo/training sketch. The buggy is always fully stopped when it shoots: SHOOT (gate open, drive neutral) -> cooldown -> DRIVE one segment -> cooldown -> SHOOT again. "Balls per pass" shots include both endpoints of the net length - 2 per pass fires one at the near end and one at the far end, 4 per pass fires at the near end, 1/3, 2/3, and the far end. The last shot of a pass is already at the far end, so direction flips and the next pass's first shot fires right there with no extra drive (the one exception is a 1-ball pass, which still drives the full length after its single shot since there's no second endpoint to space against). Flywheels spin up once and stay spinning through the whole run. Gate open time (500ms) and the post-shoot pause (300ms) are fixed in code, not user-adjustable - both tested fine and didn't need sliders.
  ...
  - **Forward speed / Reverse speed are separate sliders**, not one shared Drive Power. Field testing on 2026-08-16 confirmed reverse runs roughly twice as fast as forward at the same commanded percent on this ESC - same pulse value, clearly different real speed. Forward: min 40%, default 60%. Reverse: min 30%, default 30%. The two defaults are picked to feel roughly speed-matched in practice; tune independently from here.
  - **Direction-change double-tap** (fixed, not exposed as sliders): the first drive segment after an in-run direction flip goes neutral hold -> brief arm-tap pulse in the new direction -> neutral again -> the real pulse that actually drives. This was pulled out on 2026-08-16 once separate forward/reverse speed sliders existed, on the theory that slowing reverse down would fix the "doesn't reverse" symptom too - it didn't. Field test confirmed the Traverse Test's own Reverse toggle drove fine on its own (it never transitions from forward within a session), but real Train mode's in-run flip silently failed to move again without the double-tap, so it's back - same values as before (500ms settle, 300ms arm-tap), just not shown as sliders since they're not something that needs day-to-day tuning. `drivePulseFor()` is the one place that picks forward vs. reverse speed for any drive pulse, used by both this sequence and ordinary same-direction segments.
  - Main screen: balls to fire, balls per pass, flywheel power + spin, forward speed, reverse speed, traverse time (default 6.0s - how long a full end-to-end drive takes, still not measured against the real net, needs a bench test).
  - Advanced Settings holds the post-drive pause, a **pass change pause** (default 1000ms - covers the pass-boundary gap where the last shot of one pass and the first shot of the next land on the same physical spot with no drive between them), and a **Traverse Test** tool with its own Forward/Reverse toggle for isolating drive-only calibration in either direction, no ball or gate involved.
  - Serial Monitor logs each shot fired, each direction flip, and each drive segment's direction/pulse/target duration - useful for confirming the sequence is doing what you expect when it's hard to tell by eye. Same heartbeat safety stop as the manual sketch. Only one sketch runs on the ESP32 at a time - flash whichever one you need for that session.
